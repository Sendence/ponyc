...
BENCHMARK_DEFINE_F(HeapBench, HeapAlloc)(benchmark::State& st) {
  ...
  while (st.KeepRunning()) {
    ...
    if(st.range(0) > HEAP_MAX)
    {
      ...
      st.ResumeTiming();
      ponyint_heap_alloc_small(actor, &_heap, ponyint_heap_index(st.range(0)), false);
      st.PauseTiming();
    }
    ponyint_heap_destroy(&_heap);
    ponyint_heap_init(&_heap);
...
BENCHMARK_DEFINE_F(HeapBench, HeapFinal)(benchmark::State& st) {
  pony_actor_t* actor = (pony_actor_t*)0xDBEEFDEADBEEF;
...
BENCHMARK_REGISTER_F(HeapBench, HeapFinal)->RangeMultiplier(2)->Ranges({{32, 1024<<10}});
